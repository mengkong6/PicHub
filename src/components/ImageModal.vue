  ...
  await Promise.all(
    upload_list.value.map((e, i) => {
      ...
      return new Promise((resolve) => {
        let filename = type == 1 ? `${e.name}${e.ext}` : `${e.name}.jpeg`
        ...
        setTimeout(() => {
          ...
          `,
            ...
            .then((res: any) => {
              ...
              e.cdn_url = `${github_config.cdn}/gh/${github_config.owner}/${github_config.repoPath}@master/${folder.value}/${filename}`
...
const addHistory = (e: any) => {
  ...
  if(history_list.value.length >= 5){
    history_list.value.pop()
  }
      ...
      <div v-for="item in history_list" :key="item.name" class="item">
        ...
        <div class="info">
          ...
          <div class="tag-box">
            ...
            <span
              v-if="item.cdn_url"
              v-bind:data-clipboard-text="GetHtmlText(item.cdn_url)"
              @click="CopyText()"
              class="tag copy-btn"
              >HTML</span
            >
